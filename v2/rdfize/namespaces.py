...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    def __init__(self): 
        ...
        self.bibliographicCitation = self.registerAnnotationProperty("bibliographicCitation", hidden=True)  # hidden: only used in widoco descr of ontology
        self.hasVersion =   self.registerDatatypeProperty("hasVersion", hidden=False)   # visible, used to describe terminologies 
        self.isReferencedBy = self.registerDatatypeProperty("isReferencedBy", hidden=False)     # used as is
