...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
        #
        # Output example:
        # sibilo:Query_002 a sh:SPARQLExecutable ;
        # rdfs:comment "Count of publication citations" ; 
        # sh:select "select (count(*) as ?cnt) where { ?cl rdf:type / rdfs:subClassOf fabio:Expression . }" ; 
        # .
        #
        lines = list()
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
        ...
        f_in = open("../sparql_queries/query-list.txt", "r")
        isFirstId = True
        query: Query = Query()
