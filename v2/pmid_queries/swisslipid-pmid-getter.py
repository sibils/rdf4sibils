...
#-------------------------------------------------
    ...
    if len(sys.argv) < 2 : 
        print("Error, usage is : python swisslipid-pmid-getter.py <output_dir>")
        sys.exit(1)
    ...
    query = """
        ...
        ?reaction SWISSLIPID:citation ?pmid .
        }
    """

    sparql_service = "https://beta.sparql.swisslipids.org/sparql"

    log_it("Querying SwissLipid SPARQL service...", sparql_service)
    client = EndpointClient(sparql_service)
    response = client.run_query(query)
    log_it("Retrieving response...")
    if not response.get("success"):
        log_it(f"ERROR while running query to get SwissLipid pmids", response.get("error_type"))
        log_it(response.get("error_msg"))
        sys.exit(2)
    rows = response.get("results").get("bindings")
    fileout = outdir + "swisslipid-pmid.txt"
    log_it("Saving pmids...")
    f_out = open(fileout, "w")
    for row in rows:
        # value example: https://pubmed.ncbi.nlm.nih.gov/14685263
        iri = row["pmid"]["value"]
        pmid = iri[32:]
        # print(iri, pmid)
