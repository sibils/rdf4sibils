...
DB_DIR=$virt_base/sibils_database
INI_FILE=$virt_ini
...
stop() {
    ...
    if pgrep $DAEMON; then
        ...
        $ISQL 1112 dba $DBA_PW exec="checkpoint;"
        echo "$(date) requesting shutdown"
        $ISQL 1112 dba $DBA_PW exec="shutdown;"
        sleep 5
...
start() {
    ...
    echo -n "$(date) checking virtuoso listening on 1112 "
    while true; do
        echo -n "."
        isqlok=$(netstat -plant 2> /dev/null | grep 1112 | grep virtuoso | grep LISTEN | wc -l)
        if [ "$isqlok" == "1" ]; then break; fi
        sleep 5
    done
    echo " OK"

    echo -n "$(date) checking virtuoso listening on 8891 "
    while true; do
    echo -n "."
    webok=$(netstat -plant 2> /dev/null | grep 8891 | grep virtuoso  | wc -l)
