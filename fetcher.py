...
# ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
        self.props = get_properties(prop_file)
        # set instance variables in this class
        self.ftp_server = self.props.get("ftp_server")
        self.ftp_dir_dict = dict()
        self.ftp_dir_dict["bib"] = self.props.get("ftp_bib_dir")
        self.ftp_dir_dict["sen"] = self.props.get("ftp_sen_dir")
        self.ftp_dir_dict["ana"] = self.props.get("ftp_ana_dir")
        self.chunks_dir = self.props.get("chunks_dir")
        self.rdf_dir = self.props.get("rdf_dir")
        self.collection = self.props.get("collection")
        self.subcoll = self.props.get("subcoll")

    # ------------------------------------------------------------------
    def get_chunk_names_from_ftp(self):
    # ------------------------------------------------------------------
        ftp_dir = self.ftp_dir_dict.get("bib")
        log_it("INFO", "Getting chunk names from parameters defined in rdfizer.properties")
        log_it("INFO", "param ftp_server:", self.ftp_server)
        log_it("INFO", "param bib directory:", self.ftp_dir)
        chunk_list=list()
    ...
    # ------------------------------------------------------------------
    def fetch_by_ftp(self, ftp_dir, ftp_file, target_dir):
    # ------------------------------------------------------------------
        ftp = ftplib.FTP(self.ftp_server)
        ftp.login()
    ...
    # ------------------------------------------------------------------
    def fetch_chunk(self, chunk_name):
    # ------------------------------------------------------------------
        log_it("INFO", "Fetching chunk ", chunk_name)
        for key in self.ftp_dir_dict:
            gz_file = key + "_" + chunk_name + ".json.gz"
            if not self.chunks_dir.endswith("/"): self.chunks_dir += "/"
            target_dir = self.chunks_dir + chunk_name + "/"
            self.fetch_by_ftp(self.ftp_dir_dict[key], gz_file, target_dir)
            gunzip(target_dir + gz_file)


    # ------------------------------------------------------------------
    def explode_bib(self, chunk_name):
    # ------------------------------------------------------------------
        log_it("INFO", "Exploding bib file for", chunk_name)

        # read json object in bib file
        chunk_dir = self.get_chunk_dir(chunk_name)
        ...
        for doc in obj["article_list"]:
            id = doc["_id"]
            doc_set.add(id)
            subdir_set.add(self.get_pmc_subdir(id))

        self.save_doc_set(chunk_dir, doc_set)
        ...
        for doc in obj["article_list"]:
            id = doc["_id"]
            target_dir = chunk_dir + self.get_pmc_subdir(id) + "/"
    ...
    # ------------------------------------------------------------------
    def explode_sen(self, chunk_name):
    # ------------------------------------------------------------------
        ...
        doc_set = self.load_doc_set(chunk_dir)
    ...
    # ------------------------------------------------------------------
    def explode_ana(self, chunk_name):
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    def get_rebuilt_annotated_publi_object(self, chunk_dir, pub_id):
    # ------------------------------------------------------------------
        #log_it("INFO", "Rebuilding annotated publi", pub_id)

        root_filename = chunk_dir + self.get_pmc_subdir(pub_id) + "/" + pub_id   
    ...
    # ------------------------------------------------------------------
    def save_annotated_publications(self, chunk_name, format="pickle"):
    # ------------------------------------------------------------------
        ...
        for id in doc_set:
            publi = self.get_rebuilt_annotated_publi_object(chunk_dir, id)
            target_dir = chunk_dir + self.get_pmc_subdir(id) + "/"
            if format == "json":
    ...
    # ------------------------------------------------------------------
    def load_annotated_publications(self, chunk_name, format="pickle"):
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    def prepare_chunk(self, chunk_name):
    # ------------------------------------------------------------------
        self.fetch_chunk(chunk_name)
        self.explode_bib(chunk_name)
        self.explode_sen(chunk_name)
        self.explode_ana(chunk_name)
        self.save_annotated_publications(chunk_name)


    # ------------------------------------------------------------------
    def publi_can_be_rdfized(self, publi, pmcid):
    # ------------------------------------------------------------------
        id = publi.get("_id")
        if id == "":
    ...
    # -----------------------------------------------------------------------
    def save_rdf_files(self, chunk_name):
    # -----------------------------------------------------------------------
        chunk_rdf_dir = self.get_chunk_rdf_dir(chunk_name)
        chunk_dir = self.get_chunk_dir(chunk_name)
        doc_set = self.load_doc_set(chunk_dir)
        ...
        for pmcid_subset in pmcid_subsets:
            ...
            for pmcid in pmcid_subset:
                ...
                jsonfile = chunk_dir + self.get_pmc_subdir(pmcid) + "/" + pmcid + ".pickle"
                #log_it("DEBUG", "Reading publi", jsonfile, "file no.", pub_no)
                f_in = open(jsonfile, 'rb')
                publi = pickle.load(f_in)
                f_in.close()
                if self.publi_can_be_rdfized(publi, pmcid):
        ...
        # self.run_cmd("lz4 --rm -m " + chunk_rdf_dir + "*.ttl")

        # rather compress with gzip because virtuoso knows how to deal with this format
        self.run_cmd("gzip --fast " + chunk_rdf_dir + "*.ttl")

    # ------------------------------------------------------------------
    def process_chunk(self, chunk_name):
    # ------------------------------------------------------------------
        if self.chunk_rdf_dir_exists(chunk_name):
            ...
            self.prepare_chunk(chunk_name)
            self.save_rdf_files(chunk_name)
            self.clean_chunk(chunk_name)


    # ------------------------------------------------------------------
    def get_pmc_subdir(self, pmcid):
    # ------------------------------------------------------------------
        # return 2 last characters i.e. PMC2193330 => 30
        return pmcid[-2:]


    # ------------------------------------------------------------------
    def save_doc_set(self, chunk_dir, doc_set):
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    def load_doc_set(self, chunk_dir):
    # ------------------------------------------------------------------
        filename = chunk_dir + "doc_set.pickle"
        f_in = open(filename, "rb")
    ...
    # ------------------------------------------------------------------
    def get_chunk_dir(self, chunk_name):
    # ------------------------------------------------------------------
        if not self.chunks_dir.endswith("/"): self.chunks_dir += "/"
        chunk_dir = self.chunks_dir + chunk_name + "/"
        if not os.path.exists(chunk_dir): os.makedirs(chunk_dir)
        return chunk_dir


    # ------------------------------------------------------------------
    def get_chunk_rdf_dir(self, chunk_name):
    # ------------------------------------------------------------------
        #log_it("DEBUG", "get_chunk_rdf_dir()", "rdf_dir", rdf_dir)
        if not self.rdf_dir.endswith("/"): self.rdf_dir += "/"
        chunk_rdf_dir = self.rdf_dir + chunk_name + "/"
        if not os.path.exists(chunk_rdf_dir): os.makedirs(chunk_rdf_dir)
        return chunk_rdf_dir


    # ------------------------------------------------------------------
    def chunk_rdf_dir_status_is_loading(self, chunk_name):
    # ------------------------------------------------------------------
        dir = self.get_chunk_rdf_dir(chunk_name)
        return os.path.isfile(dir + "LOADING")

    # ------------------------------------------------------------------
    def chunk_rdf_dir_status_is_loaded(self, chunk_name):
    # ------------------------------------------------------------------
        dir = self.get_chunk_rdf_dir(chunk_name)
        return os.path.isfile(dir + "LOADED")

    # ------------------------------------------------------------------
    def chunk_rdf_dir_status_is_load_error(self, chunk_name):
    # ------------------------------------------------------------------
        dir = self.get_chunk_rdf_dir(chunk_name)
        return os.path.isfile(dir + "LOAD_ERROR")

    # ------------------------------------------------------------------
    def set_chunk_rdf_dir_loading_status(self, chunk_name):
    # ------------------------------------------------------------------
        dir = self.get_chunk_rdf_dir(chunk_name)
        open(dir + "LOADING", 'a').close()

    # ------------------------------------------------------------------
    def set_chunk_rdf_dir_loaded_status(self, chunk_name):
    # ------------------------------------------------------------------
        dir = self.get_chunk_rdf_dir(chunk_name)
        open(dir + "LOADED", 'a').close()

    # ------------------------------------------------------------------
    def set_chunk_rdf_dir_load_error_status(self, chunk_name):
    # ------------------------------------------------------------------
        dir = self.get_chunk_rdf_dir(chunk_name)
        open(dir + "LOAD_ERROR", 'a').close()

    # ------------------------------------------------------------------
    def chunk_dir_exists(self, chunk_name):
    # ------------------------------------------------------------------
        if not self.chunks_dir.endswith("/"): self.chunks_dir += "/"
        chunk_dir = self.chunks_dir + chunk_name + "/"
    ...
    # ------------------------------------------------------------------
    def chunk_rdf_dir_exists(self, chunk_name):
    # ------------------------------------------------------------------
        if not self.rdf_dir.endswith("/"): self.rdf_dir += "/"
        chunk_rdf_dir = self.rdf_dir + chunk_name + "/"
    ...
    # ------------------------------------------------------------------
    def clean_chunk(self, chunk_name):
    # ------------------------------------------------------------------
        chunk_dir = self.get_chunk_dir(chunk_name)
        tmp_dir = "./tmp_" + chunk_name + "/"
        if not os.path.exists(tmp_dir): os.makedirs(tmp_dir)
        self.run_cmd("mkdir -p " + tmp_dir)
        self.run_cmd("mv " + chunk_dir + "*.gz " + tmp_dir)
        self.run_cmd("rm -rf " + chunk_dir + "*")
        self.run_cmd("mv " + tmp_dir + "*.gz " + chunk_dir)
        self.run_cmd("rmdir " + tmp_dir)
...
# ------------------------------------------------------------------
        ...
        t0 = datetime.datetime.now()
        log_it("INFO", "Processing chunk", chunk_name, fetcher.chunks_dir)
        fetcher.process_chunk(chunk_name)
...
# ------------------------------------------------------------------
    ...
    fetcher.chunks_dir = "./data.test/chunks" # for test
    fetcher.rdf_dir = "./data.test/ttl" # for test

# ------------------------------------------------------------------
    if sys.argv[2] == "ftp_nlst": # for test
# ------------------------------------------------------------------
        list = fetcher.get_chunk_names_from_ftp()
        print("Found", len(list), "chunks")

# ------------------------------------------------------------------
    elif sys.argv[2] == "fetch_chunk": # for test
# ------------------------------------------------------------------
        fetcher.fetch_chunk("pmc23n0023")

# ------------------------------------------------------------------
    elif sys.argv[2] == "explode_bib": # for test
# ------------------------------------------------------------------
        fetcher.explode_bib("pmc23n0023")

# ------------------------------------------------------------------
    elif sys.argv[2] == "explode_sen": # for test
# ------------------------------------------------------------------
        fetcher.explode_sen("pmc23n0023")

# ------------------------------------------------------------------
    elif sys.argv[2] == "explode_ana": # for test
# ------------------------------------------------------------------
        fetcher.explode_ana("pmc23n0023")

# ------------------------------------------------------------------
    elif sys.argv[2] == "rebuild_one_publi": # for test
# ------------------------------------------------------------------
        publi = fetcher.get_rebuilt_annotated_publi_object("./dir_test/fetch_by_ftp/pmc23n0021/", "PMC2172800")
...
# ------------------------------------------------------------------
        fetcher.save_annotated_publications("pmc23n0023")

# ------------------------------------------------------------------
    elif sys.argv[2] == "load_publi": # for test
# ------------------------------------------------------------------
        fetcher.load_annotated_publications("pmc23n0023")

# ------------------------------------------------------------------
    elif sys.argv[2] == "prepare_chunk": # for test
# ------------------------------------------------------------------
        # delete chunk data
        os.system("rm -rf " + fetcher.chunks_dir)
        # build it
        t0 = datetime.datetime.now()
        chunk_name = "pmc23n0023"
        fetcher.prepare_chunk(chunk_name)
        # load it
        fetcher.load_annotated_publications(chunk_name)
...
# ------------------------------------------------------------------
        ...
        fetcher.save_rdf_files(chunk_name)
        log_it("DEBUG", "build and load time", duration_since=t0)
...
# ------------------------------------------------------------------
        ...
        fetcher.clean_chunk(chunk_name)
